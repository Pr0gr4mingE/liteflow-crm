...
export async function deletarClientePfAction(id: string): Promise<IRespostaDTO> {
  try {
    ...
    const resposta = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/cliente-pf?id=${id}`,
      {
        method: "DELETE",
        headers: { 
    ...
    if (!resposta.ok) {
      ...
      return { 
        sucesso: false, 
        mensagem: errorData?.mensagem || "Erro na API ao deletar cliente PF." 
      };
    }

    return (await resposta.json()) as IRespostaDTO;
  } catch (error) {
    console.error("[Action Error] Erro ao deletar cliente PF:", error);
