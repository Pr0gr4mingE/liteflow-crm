...
export async function deletarClientePjAction(id: string): Promise<IRespostaDTO> {
  try {
    ...
    if (!resposta.ok) {
      ...
      return { 
        sucesso: false, 
        mensagem: errorData?.mensagem || "Erro na API ao deletar cliente PJ." 
      };
    }

    return (await resposta.json()) as IRespostaDTO;
  } catch (error) {
    console.error("[Action Error] Erro ao deletar cliente PJ:", error);
