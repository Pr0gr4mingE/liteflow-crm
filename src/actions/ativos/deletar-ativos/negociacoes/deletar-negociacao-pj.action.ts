...
export async function deletarNegociacaoPjAction(id: string): Promise<IRespostaDTO> {
  try {
    ...
    const resposta = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/negociacao-pj?id=${id}`,
      {
        method: "DELETE",
        headers: { 
    ...
    if (!resposta.ok) {
      ...
      return { 
        sucesso: false, 
        mensagem: errorData?.mensagem || "Erro na API ao deletar negociação corporativa." 
      };
    }

    return (await resposta.json()) as IRespostaDTO;
  } catch (error) {
    console.error("[Action Error] Erro ao deletar negociação PJ:", error);
