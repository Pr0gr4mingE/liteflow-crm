...
export function FormNegociacaoPf() {
  ...
  clientes,
  (c) => c.nome, // Para PJ, seria (c) => c.razaoSocial
  (c) => c.id
);
  ...
  const handleSubmit = async (formData: FormData) => {
    ...
    if (sucesso) {
      console.log("Negociação PF criada com Sucesso!");
  ...
  return (
    <form action={handleSubmit} className="space-y-4">
      <div className="flex flex-col gap-1 mb-4 relative">
        <label htmlFor="busca-cliente" className="text-sm font-medium text-gray-700">
          Selecione o Cliente *
        </label>
        ...
        {autocomplete.isOpen && !autocomplete.idSelecionado && (
          <ul className="absolute z-10 top-[70px] left-0 w-full bg-white border border-gray-200 rounded-md shadow-lg max-h-48 overflow-y-auto">
            {autocomplete.itensFiltrados.length > 0 ? (
              ...
              <li className="p-2 text-sm text-gray-500 text-center">Nenhum cliente encontrado.</li>
            )}
          </ul>
        )}
      </div>
